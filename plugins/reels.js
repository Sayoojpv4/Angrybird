...
Asena.addCommand({pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram" , dontAddCommandList: true }, async (message, match) => {
   ...
   const { status, type, data } = await instagram(match[1], 'SuperSmart')
    if (!status) return await message.sendMessage('✅️Example : https://www.instagram.com/p/CBACL5vAlL3/?utm_medium=copy_link\n\n*Change /reel/ to /p/ Then give Command .insta*\n\n*Any doubt ask to owner*')
